...
class SendLoginEmailViewTest(TestCase):
    ...
    def test_adds_success_message(self):
        # follow is used to follow the redirected URL
        response = self.client.post(
            '/accounts/send_login_email',
            data={
                'email': 'edith@example.com'}, follow=True)
    ...
    def test_adds_success_message_with_mocks(self, mock_messgaes):
        response = self.client.post(
            '/accounts/send_login_email',
            data={
                'email': 'edith@example.com'})
        expected =\
            "Check your email, we've sent you a link you can use to log in."
        self.assertEqual(
            mock_messgaes.success.call_args, call(
                response.wsgi_request, expected))
        # print(mock_messgaes.success.call_args)
        # print(type(response.wsgi_request.__dict__))
